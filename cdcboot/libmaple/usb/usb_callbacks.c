...
#endif
#if 1
    {(uint8*)&usbVcomDescriptor_iManufacturer,USB_DESCRIPTOR_STRING_LEN(18)},
...
//FIFO:
#define	VCOM_RX_MASK (VCOM_RX_BUFLEN-1)
uint8 vcomFiFoRx[VCOM_RX_BUFLEN];
volatile uint32 rxBufIn  = 0;	// from host-PC.
volatile uint32 rxBufOut = 0;	//   to usbReceiveBytes.
volatile uint8  rxBufPending = 0;
...
{
  uint8  vcomBufferRx[64];
  uint32 rx_size;
  uint32 i,in,fifo_size;
  ...
  rx_size = GetEPRxCount(VCOM_RX_ENDP);
  SetEPRxStatus(VCOM_RX_ENDP,EP_RX_NAK);

  PMAToUserBufferCopy(vcomBufferRx,VCOM_RX_ADDR,rx_size);

  in = rxBufIn;
  for(i=0;i<rx_size;i++) {
  	vcomFiFoRx[in] = vcomBufferRx[i];
  	in++;in &= VCOM_RX_MASK;
	if(in == rxBufOut) break;
  }
  rxBufIn = in;

  fifo_size = (rxBufIn - rxBufOut) & VCOM_RX_MASK;
  if(fifo_size < (VCOM_RX_BUFLEN - 64)) {
...
{
    return (rxBufIn - rxBufOut) & VCOM_RX_MASK;
}
...
{
    uint32 fifo_size = usbBytesAvailable();		// holding chars in FIFO.
	uint32 i,out;
	...
	out = rxBufOut;
	for (i=0;i<len;i++) {
		recvBuf[i] = vcomFiFoRx[out];
 	 	out++;out &= VCOM_RX_MASK;
	}
	rxBufOut = out;

	//---------- usb rx int pending ?
	if(rxBufPending) {
	    fifo_size = usbBytesAvailable();		// holding chars in FIFO.
  		if(fifo_size < (VCOM_RX_BUFLEN - 64)) {
		    SetEPRxCount( VCOM_RX_ENDP,VCOM_RX_EPSIZE);
    		SetEPRxStatus(VCOM_RX_ENDP,EP_RX_VALID);
			rxBufPending = 0;
