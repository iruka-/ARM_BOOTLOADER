...
#include "irq.h"
#include "uart.h"
#include "monitor/usbio.h"	//void 	USBenqueue(uchar *buf,int size);
...
#define	DLVAL		((int)((double)PCLK / BPS / 16 / (1 + (double)DIVADDVAL / MULVAL)))
   	...
   	DLVAL = PCLK / 16 / (1+ 5/8) / BPS
		  = PCLK / 16 / (13 / 8) / BPS
		  = PCLK / 16 / (13 / 8) / BPS
          = PCLK / 26 / BPS

 */
		Fdiv = (PCLK / 26) / baudrate;		// CQ-FRK-NXP
...
//    	}
	...
	else if ( PortNum == 1 )
	{
    	...
    	install_irq( UART1_INT, (void *)UART1Handler, HIGHEST_PRIORITY );
