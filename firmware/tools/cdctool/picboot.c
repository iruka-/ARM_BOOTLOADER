...
#define CMD_NAME "armboot.exe"
#define DEFAULT_SERIAL	"*"
#define DEFAULT_BAUDRATE "1000000"	//"230400"

char HELP_USAGE[]={
	"* ARMboot Ver 0.4 (" __DATE__ ")\n"
	"Usage is\n"
...
//	"  -nv         ...  No-Verify.\n"
	...
	"  -sXXXX      ...  Set program start address(hex). (default=3000)\n"
};
...
static	char usb_baud[128]=DEFAULT_BAUDRATE;
...
#define	FLASH_STEP	4096
#define	ERASE_STEP	0x00020000	// 128k単位.

#else

//==STM32=====================
#define	FLASH_BASE	0x08000000
#define	FLASH_START	0x00003000
...
int	parse_ihex(int scmd,unsigned char *buf,int length)
{
	switch(scmd) {
		...
		case 0x02:	//セグメントアドレスレコード:
					...
					//		   ~~~~
					//
			adr_u = (buf[4]<<8)|(buf[5]);	// i8086のセグメントアドレス.
			adr_u <<= 4; // segレジスタの値なので 16倍する.
//			printf("adr_u=%x\n",adr_u);
			return scmd;
...
int	put_srec(int adrsize,unsigned char *buf,int length)
{
	int recsize;
	int adrs=0;
	int sum=0;
	...
	sum = 0;
	for(i=0;i<length;i++) {
//		printf(" %2d checksum add=%x,%x\n",i,buf[i],sum);
		sum += buf[i];
	}
//	printf("checksum=%x\n",sum);
	if( (sum & 0xff) != 0xff ) {
		fprintf(stderr,"%s: S-RECORD Checksum Error!(sum=%02x)\n", CMD_NAME,sum & 0xff);
		exit(EXIT_FAILURE);
	}
#endif
...
int	parse_mot(int scmd,unsigned char *buf,int length)
{
	switch(scmd) {
		case '0':
			break;
		case '1':	return put_srec(2,buf,length);
		case '2':	return put_srec(3,buf,length);
		case '3':	return put_srec(4,buf,length);
...
static	int read_mot_string(char *s)
{
	int c = s[1];
	switch(c) {
		case '0':
		...
		case '9':
			//printf("Srec %s\n",s);
			read_hex_string(s+2);	// 'S0' に続く16進をbinary化.
			parse_mot(c,databuf,dataidx);
			return 0;
		default:
...
#ifdef	SH2A
	int boot_pc = BSWAP32( buf[0] );	// SH2A
#else
	int boot_pc = BSWAP32( buf[1] );	// ARM
#endif

	if(( boot_pc >= FLASH_BASE)&&
	   ( boot_pc < (FLASH_BASE+FLASH_SIZE)) ){
		printf("reset vector: %08x\n",boot_pc);
		UsbBootTarget(boot_pc,1,vect);
	}else{
...
//	(addr,size)エリアを消去する必要があるなら 1 を返す.
int	flash_buf_check(int addr , int size)
{
	int i,f=0;
	for(i=0;i<size;i++) {
...
#ifdef	FM3
void UsbEraseTargetROM_FM3(int addr , int size)
{
	if(addr == 0) {
	  // 0x00000 - 0x1ffff の場合.
	  if(flash_start==0) {							// 0x00000- を消去する？.
		UsbEraseTargetROM(0x0000 , 0x04000);
	  }
	  if(flash_buf_check( 0x4000 , 0x04000)){		// 0x04000- を消去する？.
		UsbEraseTargetROM(0x4000 , 0x04000);
	  }
	  if(flash_buf_check( 0x8000 , 0x1c000)){		// 0x08000- を消去する？.
		UsbEraseTargetROM(0x8000 , 0x1c000);
	  }
	}else{
	  // 0x20000 - 0xfffff の場合.
		UsbEraseTargetROM(addr , size);
	}
}
#endif
/*********************************************************************
...
#ifdef	FM3
		UsbEraseTargetROM_FM3(FLASH_BASE+addr , pages);
#else
...
#ifdef	FM3
	for(i=0;i<flash_end;i+= ERASE_STEP) {
		erase_block(i,ERASE_STEP);	//128k単位.
	}
#else
...
int main(int argc,char **argv)
{
  ...
  {
	//初期化.
   if( UsbInit(verbose_mode, 0, usb_serial,usb_baud) == 0) {
...
{
//	portList = portlist_lpc13xx;	//書き換え.
}
