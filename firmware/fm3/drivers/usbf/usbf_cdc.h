...
#define USBF_CDC_CTRL_EP                         (USBF_EP0)
#define USBF_CDC_BULKOUT_EP                      (USBF_EP3)	//(USBF_EP1)
#define USBF_CDC_BULKIN_EP                       (USBF_EP1) //(USBF_EP2)
#define USBF_CDC_INTIN_EP                        (USBF_EP2)
...
extern uint8_t USBF_CDC_Request(const USBF_ST_SETUP_INFO * pSetupPkt);

/* MSC driver polling process */
extern uint8_t USBF_CDC_Polling(void);

/* MSC driver data transfer complete notification */
extern uint8_t USBF_CDC_TransComplete(uint8_t EpId, uint16_t Count, uint8_t CompleteStatus);
