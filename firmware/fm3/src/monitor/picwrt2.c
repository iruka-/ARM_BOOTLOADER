...
#define portPDOR0  *((volatile unsigned int*)(0x40033400UL))
#define portPDIR0  *((volatile unsigned int*)(0x40033300UL))

//	P00 への出力値を一時的にためる変数.
static	unsigned int portD0=0;

//	P00 への出力値を変更するマクロ : 結果は portD0 変数のみに反映.
#undef	Set_bit
#define	Set_bit(_port,_bit)	\
		if(_bit) {portD0 |=  (1<< (_port) );}	\
		else     {portD0 &= ~(1<< (_port) );}

//	P00 への出力値を実際のポートに出力する.
#define	PortOUT()	{portPDOR0 = portD0 ;}
